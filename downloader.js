...
function searchYouTubeResults(query, count = 5) {
  return new Promise((resolve) => {
    ...
    proc.stderr.on("data", (data) => {
      console.error("[yt-dlp search error]", data.toString());
    });
...
function downloadSong(queryOrUrl, onStatus, onProgress, filenameHint = null) {
  return new Promise((resolve) => {
    ...
    const target = isUrl
      ? `"${queryOrUrl}"`
      : `"ytsearch1:${queryOrUrl} official audio"`;
    ...
    if (
      ...
      fs.statSync(expectedFile).size > 10_000
    ) {
    ...
    onStatus(
      isUrl
        ...
        : `Searching YouTube & downloading "${queryOrUrl}"...`,
    );
    ...
    proc.on("close", (code) => {
      ...
      if (allFiles.length > 0) {
        return resolve({
          success: true,
          filePath: path.join(DOWNLOAD_DIR, allFiles[0]),
        });
      }
      resolve({
        success: false,
        error: `yt-dlp exited with code ${code}. Song not found or download failed.`,
      });
    });
    ...
    proc.on("error", (err) => {
      resolve({
        success: false,
        error: `Failed to run yt-dlp: ${err.message}`,
      });
    });
  });
}
